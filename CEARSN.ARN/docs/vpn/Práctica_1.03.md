...
En esta práctica NO GUIADA tenéis que crear una máquina virtual EC2 (un simple servidor con *Amazon Linux,* del tipo más pequeño posible) en vuestra subred privada (la que creasteis en la práctica anterior), y tenéis que conseguir actualizar (```sudo apt update && sudo apt upgrade -y```) la máquina.
...
3. Configurar un método de acceso a Internet para la subred privada (preferiblemente una *gateway NAT).*
